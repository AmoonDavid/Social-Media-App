...
const Actions = ({post}) => {
	...
	const handleLikeAndUnlike = async () => {
		if(!user){
			showToast("Error", "Please Login to Like or Unlike a post", "error");
		}
		if(isLiking) return;
		setIsLiking(true);
		try {
			//const res = await fetch("/api/posts/like/"+post._id, {
			const res = await fetch(import.meta.env.VITE_LIKE_API+post._id, {
				method:"PUT",
				headers:{
	...
	const handleReply = async () => {
		...
		try {
			...
			const res = await fetch(import.meta.env.VITE_REPLY_API+post._id, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
    ...
    <>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme='gray' size={"sm"} mr={3} onClick={handleReply} isLoading={isReplying}>
              Reply
            </Button>
          </ModalFooter>
        </ModalContent>
